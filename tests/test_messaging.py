def test_buyer_can_start_conversation(
    ...
    response = client.get(url_for_with_app("messaging.start", car_id=car.id), follow_redirects=True)
...
def test_send_message_creates_record_and_notification(
    logged_in_client, user_factory, car_factory, url_for_with_app,
):
    ...
    client.get(url_for_with_app("messaging.start", car_id=car.id), follow_redirects=True)

    from models import Conversation, Message, Notification
    convo = Conversation.query.first()
    response = client.post(
        url_for_with_app("messaging.thread", conversation_id=convo.id),
...
def test_outsider_cannot_view_thread(client, user_factory, car_factory, url_for_with_app):
    ...
    response = client.get(url_for_with_app("messaging.thread", conversation_id=convo.id))
