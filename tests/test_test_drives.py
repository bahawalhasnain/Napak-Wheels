...
def test_buyer_can_request_test_drive(
    ...
    response = client.post(
        url_for_with_app("test_drives.new", car_id=car.id),
        data={
            "requested_at": when,
            "duration_minutes": "30",
...
def test_seller_can_confirm_test_drive(client, user_factory, car_factory, url_for_with_app):
    ...
    client.post(url_for_with_app("test_drives.new", car_id=car.id), data={
    ...
    response = client.post(
        url_for_with_app("test_drives.confirm", drive_id=drive.id),
...
def test_past_date_is_rejected(logged_in_client, user_factory, car_factory, url_for_with_app):
    client, _buyer = logged_in_client
